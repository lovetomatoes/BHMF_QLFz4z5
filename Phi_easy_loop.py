...
logd_fit, l_cut, a = -1.,   .3,  -0.5; f_seed = 0.01
logd_fit, l_cut, a = -0.52534832,  0.12259319,  0.38407638 # -16.40309300058925
t_life = 1000
...
print('max Dt:', np.max((tz-T['t_col']))/Myr)
...
while True:
    ...
    if n_seed==len(T):
        break
    # print('Nt in seed:', Nt)
print('seeding ends, P_tot:',np.sum(dP_MBH))
while Nt>=0:
    t_point = tz - Nt*t_life
    dP_MBH_prev = dP_MBH.copy()
    dP_MBH = np.zeros(N_mf)
    # DT+= t_life
    for iM in range(N_mf):
        lbd = kernelS_MBH_M(abin_mf[iM:], M_BH[iM], t_life, 1., l_cut, d_fit)
        lbd[0] = x0
        Ps = integral(a,lbd,x0)/I_toinf
        dP_MBH[iM:] += (Ps[1: ]-Ps[ :-1])*dP_MBH_prev[iM]
        # print('lbd',lbd)
        # print('dP_lambda',(Ps[1: ]-Ps[ :-1]))
        # print('dP_lambda tot',(Ps[-1]-Ps[0]))
        # print(iM,len(lbd),N_mf)
    # exit(0)
    Nt -= 1
...
MFname = './f{:d}Phi_easyloopMF3pt{:d}'.format(int(abs(np.log10(f_seed))),int(t_life/Myr))
...
LFname = './f{:d}Phi_easyloopLF3pt{:d}'.format(int(abs(np.log10(f_seed))),int(t_life/Myr))
